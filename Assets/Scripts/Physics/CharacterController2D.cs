...
public class CharacterController2D : MonoBehaviour
{
	...
	/// <summary>
	/// If the character is moving down, he will snap to the ground if he is this close to the ground.
	/// </summary>
	public float snapToGroundThreshold;
	...
	public void Move( Vector3 deltaMovement )
	{
		...
		if (deltaMovement.y < 0f && !collisionState.below && snapToGroundThreshold > 0f)
			fakeBottomCollision(ref deltaMovement);
	...
	private void fakeBottomCollision(ref Vector3 deltaMovement) {
		var rayDistance = snapToGroundThreshold;
		var rayDirection = Vector2.down;
		var initialRayOrigin = _raycastOrigins.bottomLeft;

		// apply our deltaMovement here so that we do our raycast from the actual position we would be in if we had already moved
		initialRayOrigin += deltaMovement;

		var mask = platformMask;
		if(ignoreOneWayPlatformsThisFrame)
		...
		for( var i = 0; i < totalVerticalRays; i++ )
		{
			...
			DrawRay( ray, rayDirection * rayDistance, Color.magenta );
			var raycastHit = Physics2D.Raycast( ray, rayDirection, rayDistance, mask );
			if(raycastHit)
			{
				if (raycastHit.distance < distanceToGround) {
					distanceToGround = rayDistance = raycastHit.distance;
				}
			}
		}
		...
		if (distanceToGround <= snapToGroundThreshold) {
			deltaMovement.y -= distanceToGround;
